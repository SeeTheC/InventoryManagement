...
{
     protected $table = 'tbl_keyboard_info';
	...
	*/	
	public static function saveForm($form,$user_id)
	{
    	...
    	$m=$mid>0 ? ( KeyboardInfo::where('mid',$mid)->get() ) : array();                    		
	  	if(count($m)==0)
	  	{
	    	$tb= new KeyboardInfo($form); 
	    	$tb->mid=$mid;      
	    	$tb->by_user_id=$user_id;  
	    	KeyboardInfo::setNullables($tb);     
	    	$tb->save();      	    	
			$result["rtnId"]=$tb->id;
			return $result;
	  	}	
