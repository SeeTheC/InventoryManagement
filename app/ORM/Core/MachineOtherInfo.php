...
class MachineOtherInfo extends Model
{
    protected $table = 'tbl_machine_other_info';
	// optional : connection -> Database name
	protected $connection = 'mysql';
	protected $fillable =['attached_printer','attached_printer'];
	protected $nullable = ['attached_printer','attached_printer'];	
	...
	*/	
	public static function saveForm($form,$user_id)
	{
    	...
    	$m=$mid>0 ? ( MachineOtherInfo::where('mid',$mid)->get() ) : array();                    		
	  	if(count($m)==0)
	  	{
	    	$tb= new MachineOtherInfo($form); 
	    	$tb->mid=$mid;      
	    	$tb->by_user_id=$user_id;  
	    	MachineOtherInfo::setNullables($tb);     
	    	$tb->save();      	    	
			$result["rtnId"]=$tb->id;
			return $result;
	  	}	
